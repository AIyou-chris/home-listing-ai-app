...
async function check() {
  try {
    const { data, error } = await supabase.rpc('get_service_status'); // Just random check
    fs.writeFileSync('rpc_check.txt', 'RPC result: ' + (error ? error.message : JSON.stringify(data)));
  } catch (e) {
    fs.writeFileSync('rpc_check.txt', 'CATCH: ' + e.message);
  }
}
