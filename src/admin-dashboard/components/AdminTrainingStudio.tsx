...
} from '../../components/AIInteractiveTraining';

interface AdminTrainingStudioProps {
    demoMode?: boolean;
}
...
const AdminTrainingStudio: React.FC<AdminTrainingStudioProps> = ({ demoMode = false }) => {
    const [selectedSidekick, setSelectedSidekick] = useState<string>(ADMIN_SIDEKICKS[0].id);
    ...
    const currentSidekick = ADMIN_SIDEKICKS.find(s => s.id === selectedSidekick) || ADMIN_SIDEKICKS[0];
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {messages.length === 0 && (
                        <div className="h-full flex flex-col items-center justify-center text-slate-400">
                            ...
                            <p>Select a sidekick and start training</p>
                        </div>
                    )}
                    {messages.map(msg => (
...
export default AdminTrainingStudio;
