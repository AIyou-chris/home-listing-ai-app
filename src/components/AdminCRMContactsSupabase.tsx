...
const AdminCRMContactsSupabase: React.FC = () => {
	...
	useEffect(() => {
		const readLocalContacts = (): Contact[] => {
			try {
				const raw = localStorage.getItem('hlai_contacts')
			...
			try {
				...
				if (user) {
					...
					if (typeof listener === 'function') {
						unsubscribeContacts = listener;
					} else if (hasUnsubscribe(listener)) {
						unsubscribeContacts = () => listener.unsubscribe();
					}
				} else {
		...
		loadContacts().then((fn) => {
			stopContacts = fn ?? null;
		});
	...
	// Load notes/files and subscribe for real-time updates for the active contact
	useEffect(() => {
		...
		const init = async () => {
			try {
				const [notes, files] = await Promise.all([
					supabaseContactService.getContactNotes(actionContact.id),
					supabaseContactService.getContactFiles(actionContact.id)
				]);
				setNotesById(prev => ({ ...prev, [actionContact.id]: notes }));
				setFilesById(prev => ({ ...prev, [actionContact.id]: files }));
				// Subscribe to realtime updates
				const notesListener = await supabaseContactService.onContactNotesChange(
					actionContact.id,
					(updated) => setNotesById(prev => ({ ...prev, [actionContact.id]: updated }))
				);
				if (typeof notesListener === 'function') {
					unsubscribeNotes = notesListener;
				} else if (hasUnsubscribe(notesListener)) {
					unsubscribeNotes = () => notesListener.unsubscribe();
				}
				const filesListener = await supabaseContactService.onContactFilesChange(
					actionContact.id,
					(updated) => setFilesById(prev => ({ ...prev, [actionContact.id]: updated }))
				);
				if (typeof filesListener === 'function') {
					unsubscribeFiles = filesListener;
				} else if (hasUnsubscribe(filesListener)) {
					unsubscribeFiles = () => filesListener.unsubscribe();
				}
			} catch (error) {
	...
	const handleDelete = async (id: string) => {
		...
		const localInsert = () => {
			...
			const items: Contact[] = raw ? JSON.parse(raw) : []
		...
		try {
			...
			// Ensure UI refresh pulls local when not authed
			setContacts(await supabaseContactService.getContacts())
		} catch (error) {
			console.warn('Supabase insert failed, using local fallback:', error);
			localInsert();
			alert('Test contact added locally!');
		}
	};
	...
	const handleAddNote = async () => {
		...
		try {
			...
			setNewNote('');
			// Refresh notes list so the new note appears immediately
			...
			<div className="mb-8">
				<div className="flex items-center justify-between mb-6">
					...
					<div className="flex items-center gap-3">
						...
						<button onClick={addFakeContact} className="px-4 py-2 bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700 transition">
							<span className="material-symbols-outlined text-sm">add</span>
							Test Contact
						</button>
						<button onClick={() => handleOpen()} className="px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition">
							<span className="material-symbols-outlined text-sm">add</span>
							New Contact
						</button>
					</div>
								...
								<div className="space-y-3">
									...
									<div className="flex gap-2">
										...
										<button onClick={() => {
											...
											supabaseContactService.updateContact(actionContact.id, {
									...
									<div>
										...
										<textarea rows={4} defaultValue={actionContact.pipeline_note || ''} onBlur={e => {
											supabaseContactService.updateContact(actionContact.id, {
...
export default AdminCRMContactsSupabase;
