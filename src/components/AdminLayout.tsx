...
const AdminLayout: React.FC<AdminLayoutProps> = ({ currentView }) => {
  ...
  const {
    ...
    hasLoaded: hasMarketingLoaded,
    errorMessage: marketingError,
    refreshMarketingData,
    deleteSequence: deleteMarketingSequence,
    deleteQrCode: deleteMarketingQrCode
  } = useAdminMarketing({ autoFetch: currentView === 'admin-marketing' });
  ...
  const handleDeleteUser = (userId: string) => {
    ...
    setUsers(prev => {
      ...
      try { localStorage.setItem('adminUsers', JSON.stringify(next)); } catch (_error) { /* ignore storage errors */ }
      return next;
    });
  };
  ...
  const handleDeleteLead = (leadId: string) => {
    ...
    setLeads(prev => {
      ...
      try { localStorage.setItem('adminLeads', JSON.stringify(next)); } catch (_error) { /* ignore storage errors */ }
      return next;
    });
  };

  const handleAddLead = async (leadData: LeadFormInput) => {
      ...
      case 'admin-marketing':
        return (
          <AdminMarketingPanel
            ...
            hasLoaded={hasMarketingLoaded}
            errorMessage={marketingError}
            onRefresh={refreshMarketingData}
            onDeleteSequence={deleteMarketingSequence}
            onDeleteQrCode={deleteMarketingQrCode}
          />
        );
