				...
				<div key={feedback.id} className="bg-white rounded-lg border border-slate-200 p-4">
					<div className="flex items-start gap-3">
						<div className={`p-2 rounded-full ${
							feedback.type === 'thumbs_up' ? 'bg-green-100' :
							feedback.type === 'thumbs_down' ? 'bg-red-100' : 'bg-blue-100'
						}`}>
							<span className={`material-symbols-outlined text-sm ${
								feedback.type === 'thumbs_up' ? 'text-green-600' :
								feedback.type === 'thumbs_down' ? 'text-red-600' : 'text-blue-600'
							}`}>
								{feedback.type === 'thumbs_up' ? 'thumb_up' :
								 feedback.type === 'thumbs_down' ? 'thumb_down' : 'school'}
							</span>
						</div>
