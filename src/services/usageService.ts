...
export const addUsage = async (
  ...
  const updated: UsageRecord = {
    ...
    prompt_tokens: record.prompt_tokens + usage.prompt,
    completion_tokens: record.completion_tokens + usage.completion,
    total_tokens: record.total_tokens + usage.total,
    cost_usd: +(record.cost_usd + usage.costUsd).toFixed(4)
  };
