 ...
 */
export const addConsultation = async (consultationData: Omit<Consultation, 'id' | 'createdAt'>): Promise<string> => {
  try {
    const dataToSave = {
      ...consultationData,
      createdAt: Timestamp.now(),
...
export const getConsultations = async (): Promise<Consultation[]> => {
  try {
    ...
    const querySnapshot = await getDocs(consultationsCol);
    ...
    return consultations.sort((a, b) => b.createdAt.toMillis() - a.createdAt.toMillis());
  } catch (error) {
    console.error("Error getting consultations: ", error);
    throw new Error("Could not fetch consultations.");
  }
};